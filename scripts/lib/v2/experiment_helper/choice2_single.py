...
from scripts.lib.v2.reporter import create_report, create_report_overall
...
class SinglePremiseExperiment:
    ...
    nonents = ["non-entailment", "non-entailment."]
    ents_ja = ["含意", "含意する"]
    nonents_ja = ["含意しない", "含意しない。"]
    ...
    def make_prompt(
        ...
        if lang == "en":
            # Exclude premise and hypothesis from bind_vars
            prompt = prompt.format(**bind_vars)

            problem = f"Premise: {pr}\n" + f"Hypothesis: {con}"

            # NOTE: Use "{problem}" as a special placeholder for the problem
            if "{problem}" in prompt:
                res = prompt.format(problem=problem)
            elif "{premise}" in prompt and "{hypothesis}" in prompt:
                res = prompt.format(premise=pr.strip("."), hypothesis=con.strip("."))
            else:
                # res = prompt + "\n" + f"{problem}"  # \n" + "The answer is: "
                res = prompt + "\n" + f"{problem}\n" + "Answer: "
        elif lang == "ja":
    ...
    def ask_llm(
        ...
        **kwargs,
        ...
        if isinstance(model, dict):
            ...
            if model_name.startswith("huggingface/"):
                ...
                if "gpt-oss-120b" in model_name:
                    params = {
                        ...
                        "reasoning_effort": "high",  # TODO: Make customizable
                    }
                else:
                    params = {
                        "model": "tgi",
            ...
            elif model_name.startswith("huggingface-ip/"):
                client = openai.OpenAI(
                    base_url=model["api_base"],
                    api_key=os.getenv("HUGGINGFACE_API_KEY"),
                )
                ...
                for attempt in Retrying(
                    ...
                    with attempt:
                        if model.startswith("o1-"):
                            params = {
                                "model": model,
                                # "max_completion_tokens": self.response_max_tokens,
                                "messages": [message],
                            }
                        else:
                            params = {
    ...
    def put_answer(self, exp):
        ...
        if ans in self.ents + self.ents_ja:
            res = "entailment"
        elif ans in self.nonents + self.nonents_ja:
            res = "non-entailment"
        else:
            res = "check"
    ...
    def predict(
        ...
        if lang == "en":
            premises = df["premise_en"]
            conclusions = df["hypothesis_en"]
        elif lang == "ja":
            premises = df["premise_ja"]
            conclusions = df["hypothesis_ja"]
        else:
            raise ValueError("Invalid language specified.")

        ids = df["ID"] if "ID" in df.columns else [None] * df.shape[0]
        ...
        self.log(f"max_tokens: {self.response_max_tokens}")
