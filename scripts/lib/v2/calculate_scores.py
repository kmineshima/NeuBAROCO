...
def calculate_scores(df, no_choice3=False, choices_n=3):
    row = {}

    if no_choice3 and choices_n == 3:
        choices_n = 5
    ...
    if "syllogism-type" in df.columns:
        print("*syllogism-type*")
        sections = ["syllogism", "propositional", "extended"]
        for section in sections:
            score = calculate_accuracy(df, "syllogism-type", section, "prediction")
            row[section] = score

    if "content-type" in df.columns and "content-type-simple" not in df.columns:
        print("*content-type*")
        ...
        #     "symbol",
        # ]
        # Get tags from the DataFrame instead:
        tags = df["content-type"].unique()
    ...
    if "content-type-simple" in df.columns:
        print("*content-type-simple*")
        tags = [
            "consistent",
            "inconsistent",
            "others",
        ...
        for tag in tags:
            score = calculate_accuracy(df, "content-type-simple", tag, "prediction")
            count = len(df[df["content-type-simple"] == tag])
            row[f"{tag} ({count})"] = score
            if choices_n == 3 or choices_n == 2:
                for label in labels:
                    score = calculate_accuracy_with_label(
                        df, "content-type-simple", tag, label, "prediction"
                    )
                    count = len(
                        df[(df["content-type-simple"] == tag) & (df["gold"] == label)]
                    )
                    row[f"{tag}_{label} ({count})"] = score
    ...
    if "form" in df.columns:
        print("*form*")
        labels = df["form"].unique()
        for label in labels:
            score = calculate_accuracy(df, "form", label, "prediction")
            count = len(df[df["form"] == label])
            row[f"{label} ({count})"] = score
    ...
    if "modal" in df.columns:
        ...
        # ]
        # Get tags from the DataFrame instead:
        tags = df["modal"].unique()
        for tag in tags:
            score = calculate_accuracy(df, "modal", tag, "prediction")
            count = len(df[df["modal"] == tag])
            row[f"{tag} ({count})"] = score
            # if choices_n == 3 or choices_n == 2:
            #     for label in labels:
            #         score = calculate_accuracy_with_label(
            #             df, "modal", tag, label, "prediction"
            #         )
            #         count = len(df[(df["modal"] == tag) & (df["gold"] == label)])
            #         row[f"{tag}_{label} ({count})"] = score
            if "content-type" in df.columns:
                score = calculate_accuracy(
                    df[df["content-type"] != "abstract"], "modal", tag, "prediction"
                )
                row[f"{tag} w/o abstract ({count})"] = score
                for content_type in df["content-type"].unique():
                    score = calculate_accuracy(
                        df[df["content-type"] == content_type],
                        "modal",
                        tag,
                        "prediction",
                    )
                    count = len(
                        df[(df["modal"] == tag) & (df["content-type"] == content_type)]
                    )
                    row[f"{tag}_{content_type} ({count})"] = score

                    if "form" in df.columns:
                        # print("*form*")
                        for form_label in df["form"].unique():
                            score = calculate_accuracy(
                                df[
                                    (df["modal"] == tag)
                                    & (df["content-type"] == content_type)
                                ],
                                "form",
                                form_label,
                                "prediction",
                            ...
                            row[f"{tag}_{content_type}_{form_label} ({count})"] = score

    if "inference-pattern" in df.columns:
        ...
        # ]
        # Get tags from the DataFrame instead:
        tags = df["inference-pattern"].unique()
        for tag in tags:
            score = calculate_accuracy(df, "inference-pattern", tag, "prediction")
            count = len(df[df["inference-pattern"] == tag])
            row[f"{tag} ({count})"] = score
            if choices_n == 3:  ## or choices_n == 2:
                for label in labels:
                    score = calculate_accuracy_with_label(
                        df, "inference-pattern", tag, label, "prediction"
                    )
                    count = len(
                        df[(df["inference-pattern"] == tag) & (df["gold"] == label)]
                    )
                    row[f"{tag}_{label} ({count})"] = score

    if "content_shikishima" in df.columns:
        print("*content_shikishima*")
        sections = [
            "contentual",
            "abstract",
            "congruent",
            ["contentual", "congruent"],
            "incongruent",
        ]
        labels = ["1", "2", "3", "4", "5"]
        for section in sections:
            score = calculate_accuracy(df, "content_shikishima", section, "prediction")
            if isinstance(section, list):
                section_str = "+".join(section)
            else:
                section_str = section
            if isinstance(section, list):
                count = len(df[df["content_shikishima"].isin(section)])
            else:
                count = len(df[df["content_shikishima"] == section])
            row[f"{section_str}_acc ({count})"] = score
            # for label in ["entailment", "contradiction", "neutral"]:
            #     score = calculate_accuracy_with_label(
            #         df, "content_shikishima", section, label, "prediction"
            #     )
            #     row[f"{section}_{label}"] = score

            # for section in sections:
            score_precision = calculate_precision(
                df, "content_shikishima", section, "prediction", None, labels=labels
            )
            row[f"{section_str}_prec ({count})"] = score_precision

            # for section in sections:
            score_recall = calculate_recall(
                df, "content_shikishima", section, "prediction", None, labels=labels
            )
            row[f"{section_str}_rec ({count})"] = score_recall

    if "conversion" in df.columns:
        print("*conversion*")
        score = calculate_accuracy(df, "conversion", True, "prediction")
        count = len(df[df["conversion"] is True])
        row[f"conversion ({count})"] = score
        if choices_n == 3:
            score = calculate_accuracy(df, "conversion", False, "prediction")
            count = len(df[df["conversion"] is False])
            ...
            for label in ["entailment", "contradiction", "neutral"]:
                ...
                count = len(df[(df["conversion"] is True) & (df["gold"] == label)])
                row[f"conversion_{label} ({count})"] = score

                score = calculate_accuracy(df, "conversion", False, "prediction")
                count = len(df[(df["conversion"] is False) & (df["gold"] == label)])
                row[f"non-conversion_{label} ({count})"] = score

        if "content-type" in df.columns:
            # symbolとcongruentでconversionの場合
            target_df = df[
                (df["content-type"] == "symbol") | (df["content-type"] == "consistent")
            ]
            score = calculate_accuracy(target_df, "conversion", True, "prediction")
            count = len(target_df[target_df["conversion"] is True])
            row[f"conversion_(symbol+consistent) ({count})"] = score
            if choices_n == 3:
                for label in ["entailment", "contradiction", "neutral"]:
                    score = calculate_accuracy_with_label(
                    ...
                    count = len(
                        target_df[
                            (target_df["conversion"] is True)
                            & (target_df["gold"] == label)
                        ]
                    )
                    if count > 0:
                        row[f"conversion_(symbol+consistent)_{label} ({count})"] = score

            # incongruentでconversionの場合
            target_df = df[df["content-type"] == "inconsistent"]
            score = calculate_accuracy(target_df, "conversion", True, "prediction")
            count = len(target_df[target_df["conversion"] is True])
            row[f"conversion_inconsistent ({count})"] = score
            if choices_n == 3:
                for label in ["entailment", "contradiction", "neutral"]:
                    score = calculate_accuracy_with_label(
                        target_df, "conversion", True, label, "prediction"
                    ...
                    if count > 0:
                        row[f"conversion_inconsistent_{label} ({count})"] = score
    ...
    if "conversion_type" in df.columns:
        ...
        for tag in tags:
            ...
            row[f"conversion_(symbol+consistent)_type_{tag} ({count})"] = score
            if choices_n == 3:
                for label in ["entailment", "contradiction", "neutral"]:
                    score = calculate_accuracy_with_label(
                    ...
                    row[
                        f"conversion_(symbol+consistent)_type_{tag}_{label} ({count})"
                    ] = score
            ...
            score = calculate_accuracy(target_df, "conversion_type", tag, "prediction")
            count = len(target_df[target_df["conversion_type"] == tag])
            row[f"conversion_inconsistent_type_{tag} ({count})"] = score
            if choices_n == 3:
                for label in ["entailment", "contradiction", "neutral"]:
                    score = calculate_accuracy_with_label(
                        target_df, "conversion_type", tag, label, "prediction"
                    ...
                    row[f"conversion_inconsistent_type_{tag}_{label} ({count})"] = score

    if "atmosphere" in df.columns:
        print("*atmosphere*")
        score = calculate_accuracy(df, "atmosphere", True, "prediction")
        row["atmosphere"] = score
        if choices_n == 3:
            for label in ["entailment", "contradiction", "neutral"]:
                calculate_accuracy_with_label(
                    df, "atmosphere", True, label, "prediction"
                )
                row[f"atmosphere_{label}"] = score

    if "atmosphere-auto" in df.columns:
        print("*atmosphere-auto*")
        score = calculate_accuracy(df, "atmosphere-auto", "yes", "prediction")
        row["atmosphere-auto"] = score
        if choices_n == 3:
            for label in ["entailment", "contradiction", "neutral"]:
                calculate_accuracy_with_label(
                    df, "atmosphere-auto", "yes", label, "prediction"
                )
                row[f"atmosphere-auto_{label}"] = score
